...
$valid = $this->validate($request, [
        ...
        'email' => 'required|unique:busdev,email'
    ]);
if (!$valid)
    { 
$masuk = array('name' => $request->name, 'email' => $request->email ); 
Busdev::create($masuk);
Log::create(['email' => Auth::guard('karyawan')->user()->email, 'table_action'=>'busdev' ,'action' => 'insert', 'data' => json_encode($masuk)]);
...
{
Log::create(['email' => Auth::guard('karyawan')->user()->email, 'table_action'=>'busdev' ,'action' => 'delete', 'data' => json_encode($cek)]);
Busdev::where('id',$id)->delete();
return response()->json(['success'=>'Delete Successfully']);
    ...
    {
$get =  Busdev::whereIn('id',explode(",",$id))->get();
Log::create(['email' => Auth::guard('karyawan')->user()->email, 'table_action'=>'busdev' ,'action' => 'delete', 'data' => json_encode($get)]);
Busdev::whereIn('id',explode(",",$id))->delete();
...
$valid = $this->validate($request, [
        'name' => 'required|max:255',
        'email' => 'required|unique:busdev,email,'.$id
    ]);
 
if (!$valid)
    {
        $edit = array('name' => $request->name, 'email' => $request->email ); 
       Busdev::where('id',$id)->update($edit);
        Log::create(['email' => Auth::guard('karyawan')->user()->email, 'table_action'=>'busdev' ,'action' => 'insert', 'data' => json_encode($edit)]);
        return response()->json(['success'=>'Add Successfully']); 
    }
else
{
