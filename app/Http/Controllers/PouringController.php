...
class PouringController extends Controller
{
    /**
...
return response()->json('error', $valid);
        ...
        public function AddDocumentPouring(Request $request)
    {
$cekdata = Pouring::where('project_id',Input::get('project_id'))->first();
if (!$cekdata) {
$valid = $this->validate($request, [
        'project_id' => 'required|max:255|unique:pouring,project_id', 
        'pouring_date' => 'required|date|date_format:Y-m-d',
...
{
$masuk = array('project_id' => $request->project_id, 'pouring_date' => $request->pouring_date, 'pouring_document' => $fileName); 
File::delete($destinationPath .$cekdata->pouring_document);
Pouring::where('id',$cekdata->id)->update($masuk);
 
Project::where('id',Input::get('project_id'))->update(['status_id'=>Input::get('status')]);
Log::create(['email' => Auth::guard('karyawan')->user()->email, 'table_action'=>'Pouring' ,'action' => 'update', 'data' => json_encode($masuk)]);
return response()->json(['success'=>'Successfully']); 
}
...
else
{
 ...
 return response()->json('error', $valid);
        ...
        public function RevisiDocumentPouring(Request $request)
    { 
$valid = $this->validate($request, [ 
        'pouring_date' => 'required|date|date_format:Y-m-d',
        'pouring_document' => 'nullable|mimes:pdf,jpg,png,jpeg', 
        'status' => 'required|numeric|not_in:0'
    ]);
if (!$valid)
    {
 if(Input::file('pouring_document'))
 {
       $file = Input::file('pouring_document');
         $extension  = Input::file('pouring_document')->getClientOriginalExtension(); // getting image extension
if ($file->getSize() <= 10000000 && $file->getClientMimeType() == 'application/pdf' || $file->getSize() <= 10000000 && $file->getClientMimeType() == 'image/png' || $file->getSize() <= 10000000 && $file->getClientMimeType() == 'image/jpg' || $file->getSize() <= 10000000 && $file->getClientMimeType() == 'image/jpeg')
...
{
File::delete($destinationPath .Input::get('pouring_document_lama'));
$edit = array('pouring_date' => $request->pouring_date, 'pouring_document' => $fileName);   
Pouring::where('id',$request->id)->update($edit);
Project::where('id',Input::get('project_id'))->update(['status_id'=>Input::get('status')]);
Log::create(['email' => Auth::guard('karyawan')->user()->email, 'table_action'=>'Pouring' ,'action' => 'update', 'data' => json_encode($edit)]);
return response()->json(['success'=>'Successfully']); 
