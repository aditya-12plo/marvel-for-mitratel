...
<button class="btn btn-danger" @click="sumSelectedItems()">Approved Terpilih <i class="ft-check-circle position-right"></i></button>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsApprovalDocumentBaksBauk"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
var masuk = 
{
  ...
  'status' : 51,
  'statusmessage' : 'APPROVED DOKUMEN BAKS BAUK',
  'kata' : 'Dokumen BAKS BAUK Disetujui',
  'document' : 'DOKUMEN BAKS BAUK',
}
axios.post('/karyawan/ApprovedSITACMassal', masuk)
                    .then(response => { 
...
return hashids.decode(id); 
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'approvaldocumentbaksbauk', params: {id: this.diacak(item.id),typenya:'approval-document-baks-bauk-manager',rowDatanya:{datanya:this.dataNya,project:item} }});
            }  ,
        doFilter () {
       ...
       var startTime = this.customFormatter(this.startTime.time)
		this.$events.fire('filter-set', this.filterText ,this.infratypenya , this.towernya,startTime, this.endtime.time )
		}
		else if(!this.startTime.time && this.endtime.time)
		{
       var endtime = this.customFormatter(this.endtime.time)
		this.$events.fire('filter-set', this.filterText ,this.infratypenya , this.towernya, this.startTime.time, endtime )
		}
		else if(this.startTime.time && this.endtime.time)
		{ 
       ...
       var startTime = this.customFormatter(this.startTime.time)
      var endtime = this.customFormatter(this.endtime.time)
		this.$events.fire('filter-set', this.filterText ,this.infratypenya , this.towernya, startTime, endtime )
		}
		}
		else
		{
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsApprovalDocumentBaksBauk').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
