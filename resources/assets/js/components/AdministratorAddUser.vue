...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal -->
        ...
        <template slot="header" ><h4>Create New Administrator Website</h4></template>
        <template slot="body" >
            <form method="POST" action="" @submit.prevent="submitBooks()">
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="suspend">Account Level</label>
...
<!-- @view --->
        ...
        <template slot="header" ><h4>View Administrator Website</h4></template>
        <template slot="body" >
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="suspend">Level :  {{forms.level}}</label>
        ...
        <template slot="body" >
            ...
            <form method="POST" action="" @submit.prevent="updateItem()">
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="suspend">User Level</label>
                        <select class="form-control border-input" v-model="forms.level" required >
						<option value="">Choose One</option>
						<option v-for="pilih in pilihan"
        :selected="pilih == forms.level ? 'selected' : ''"
        :value="pilih">
       {{ pilih }}
    </option>
						</select>
...
import BooksCustomActions from '../Button/BooksCustomActions'
Vue.use(VueEvents)
...
export default {
  ...
  data () {
    ...
    closeBtn: true,
    formErrors:{},
	pilihan: ['USER','ADMINISTRATOR'],
    errors: new Errors() ,
    forms:new CrudForm({index:'',  id:'' , name:'' , email:'' , password:'' , level:'' , created_at:''}) ,
  ...
  resetOptions() {
    ...
    submitBooks() {
            axios.post('/kevler/kevlerAdmin', this.forms)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
   ...
   updateItem() {
            axios.put('/kevler/kevlerAdmin/'+ this.forms.id , this.forms)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
            ...
            destroyItem(kode){
                axios.delete('/kevler/kevlerAdmin/'+ kode)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/kevler/kevlerAdmin').then((response) => {
                    this.dataNya = response.data;
                    this.loading = false
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
