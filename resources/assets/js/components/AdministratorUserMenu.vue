...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal--->
        ...
        <template slot="header" ><h4>Create New User Menu</h4></template>
        <template slot="body" >
  ...
  <!-- form Group -->
            ...
            <div v-show="loading2"><i class="fa fa-spinner fa-spin"></i> Loading Users</div>
            <select v-model="forms.user_id" class="form-control" required>
             <option value="">--Choose One--</option>
            <option v-for="pilihan in pilihannya" :value='pilihan'>{{ pilihan.name }}</option>
            </select>
                        <span class="label label-danger" v-for="error of errorNya['user_id']">
                        {{ error }}
                    </span> 
          <span class="label label-danger" v-for="error of errorNya['user_id']">
                        {{ error }}
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="menu_name">Menu Access :  </label>
                        <br>
<table class="table" border="1">
<tr><td align="center">Menu Header</td><td align="center">SUb Menu</td></tr>
<tr v-for="menu in menuHeader">
<td><label><input type="checkbox" v-model="rights" :value="menu.id" id="menu.id"> {{menu.menu_name}}</label></td>
<td><div v-for="menuSub in menuDetail" v-if="menuSub.menu_id == menu.id"> <label><input type="checkbox" v-model="rights_detail" :value="menuSub.id" id="menuSub.id"> {{menuSub.sub_menu_name}}</label><br></div></td>
</tr>
...
<!-- @view --->
        ...
        <template slot="header" ><h4>View User Menu</h4></template>
        <template slot="body" >
                <div class="modal-body">

                    <!-- form Group -->
                    <div class="form-group">
                        <label for="menu_name">Users Name :  {{forms.name}}</label>
                      
                    </div>

                    <!-- form Group -->
                    <div class="form-group">
                        <label for="menu_url">Users Email :  {{forms.email}}</label>
...
<table class="table" border="1">
<tr><td align="center">Menu Header</td><td align="center">SUb Menu</td></tr>
<tr v-for="(menu,index) in menuHeader">
<td><label v-if="menu.id == forms.rights.split(',')[index]">{{menu.menu_name}}</label><label v-else style="color: red;">{{menu.menu_name}}</label></td>
<td><div v-for="(menuSub,index) in menuDetail" v-if="menuSub.menu_id == menu.id"><label v-if="menuSub.id == forms.rights_detail.split(',')[index]">{{menuSub.sub_menu_name}}</label><label v-else style="color: red;">{{menuSub.sub_menu_name}}</label><br></div></td>
</tr>
<tr><td align="center"><label style="color: red;">Red No Active</label></td><td align="center"><label>Black Is Active</label></td></tr>
</table>
                    </div>
...
<!-- @update --->
        ...
        <template slot="header" ><h4>Edit User Menu</h4></template>
        <template slot="body" >
...
<!-- form Group -->
                    <div class="form-group">
           <label for="broker_code">User Name : {{forms.name}}</label>
                    </div>
...
<tr><td align="center">Menu Header</td><td align="center">Sub Menu</td></tr>
<tr v-for="(menu,index) in menuHeader">
<td><label><input type="checkbox" name="rights" v-bind:value="menu.id" :checked="menu.id == forms.rights.split(',')[index]" id="menu.id"> {{menu.menu_name}}</label></td>
<td><div v-for="(menuSub,index) in menuDetail" v-if="menuSub.menu_id == menu.id"> <label><input type="checkbox" name="rights_detail" v-bind:checked="menuSub.id == forms.rights_detail.split(',')[index]" v-bind:value="menuSub.id" id="menuSub.id"> {{menuSub.sub_menu_name}}</label><br></div></td>
</tr>
 ...
 <!-- @delete --->
        ...
        <template slot="header" ><h4>Delete Menu Master</h4></template>
        <template slot="body" >
...
import AdministratorUserMenuDetail from './AdministratorUserMenuDetail'
Vue.use(VueEvents)
...
export default {
  components: {
    ...
    'app-detail': AdministratorUserMenuDetail,
  ...
  data () {
    ...
    formErrors:{},
    pilihannya:[{id:'',name:''}],
    menuHeader:[{id:'',menu_name:''}],
    menuDetail:[{id:'',menu_id:'',sub_menu_name:''}],
    errors: new Errors() ,
    forms:new CrudForm({index:'',  id:'', name:'' , email:'' , rights:'' , rights_detail:'' , created_at:''}) ,
  ...
  methods: {  
  sumSelectedItems() {
   this.ttl = this.$refs.vuetable.selectedTo;
   ...
   var check = confirm("Are you sure you want to delete this row?");  
                ...
                this.join_selected_values = this.ttl.join(","); 
                //console.log(join_selected_values);
                
                    axios.delete('/kevler/kevlerMenuUserDeleteAll/'+ this.join_selected_values)
                    .then(response => {
                this.errorNya = '';
  ...
  resetOptions() {
          ...
          this.$refs.toast.showToast(pesan, {
            ...
            createItem() {
                ...
                this.modal.set('create', true);
                        ...
                        selectUser() {
      this.loading2 = true;
                axios.get('/kevler/kevlerMenuUserGet?token=' + this.token).then((response) => {
        ...
        if (! _.isEmpty(error.response)) {
          ...
          else
          {
                        ...
                        selectMenuDetail() {
                axios.get('/kevler/kevlerGetMenuDetail?token=' + this.token).then((response) => {
    ...
    submitBooks() {
var data = {user_id:this.forms.user_id['id'],rights:this.rights,rights_detail:this.rights_detail};

            axios.post('/kevler/kevlerMenuUser', data)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
...
var choices = [];
var choices2 = [];
var els = document.getElementsByName('rights');
var els2 = document.getElementsByName('rights_detail');
...
var data =  {rights:choices,rights_detail:choices2};


            axios.put('/kevler/kevlerMenuUser/'+ this.forms.id , data)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
            ...
            destroyItem(kode){
                axios.delete('/kevler/kevlerMenuUser/'+ kode)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/kevler/kevlerMenuUser').then((response) => {
                    this.dataNya = response.data;
                    this.loading = false
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
