...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <div class="content-header" align="center">Form Penambahan Dokumen Rfi & Baut Project ID {{this.rowDatanya.project.projectid}}</div>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen Fence & Yard
</button>
 ...
 <date-picker :date="baut_date" :option="option"></date-picker>
  <div class="help-block"><ul role="alert"><li v-for="error of errorNya['baut_date']"><span style="color:red;">{{ error }}</span></li></ul></div>
                                    ...
                                    <div class="col-xl-4 col-lg-4 col-md-12 mb-1">
                                        ...
                                        <br>
<label id="projectinput8" class="file center-block">
                        <input type="file" name="file_name_baut" id="file_name_baut" v-on:change="newAvatarBaut" required="required"> 
            <span class="file-custom"></span>
                    </label>        
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , rfibautdid:'' ,  rfi_date:'' ,  rfi_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 36)
                axios.post('/karyawan/DocumentRfiBautPerbaikan', masuk)
                    .then(response => { 
  ...
  if (result.value) {
   ...
   masuk.set('status', 39)
                axios.post('/karyawan/AddDocumentRfiBaut', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
