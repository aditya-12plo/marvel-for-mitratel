...
var masuk = 
{
  ...
  'status' : 40,
  'statusmessage' : 'APPROVED DOKUMEN RFI & BAUT',
  'kata' : 'Dokumen RFI & BAUT Disetujui',
  'document' : 'DOKUMEN RFI & BAUT',
}
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'approvaldocumentcmedata', params: {id: this.diacak(item.id),typenya:'approval-document-cme-manager',rowDatanya:{datanya:this.dataNya,project:item} }});
            }  ,
        doFilter () {
