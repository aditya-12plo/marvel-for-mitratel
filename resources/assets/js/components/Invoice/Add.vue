...
window.open(routeData.href, '_blank');
  ...
  timer: 1500
            ...
            backLink() {
 this.$router.push('/documents-invoice');
            } ,
    allcap (e, o, prop) {
...
'Ooppss?',
 kata,
  ...
  if (result.value) {    
   ...
   masuk.set('statusmessage', 'SUBMIT DOKUMEN INVOICE')
   masuk.set('document', 'DOKUMEN INVOICE')
   masuk.set('status', 54)
                axios.post('/karyawan/AddDocumentInvoice', masuk)
                    .then(response => { 
