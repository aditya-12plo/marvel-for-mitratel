...
<div class="card-header-banner"> </div> 
    ...
    <section class="content-header">
      ...
      <h1 align="center">
      List User REGIONAL {{this.dataNya.regional}}
      </h1>
    </section>
...
<button class="btn btn-danger" @click="sumSelectedItems()">Hapus Terpilih <i class="fa fa-trash position-right"></i></button>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/listuserregionalaccountmanager"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
 ...
 kata,
  ...
  if (result.value) {
  	  var join_selected_values = ttl.join(","); 
                axios.delete('/karyawan/listuserDeleteAll/'+ join_selected_values)
                    .then(response => {
                this.errorNya = '';
...
return hashids.decode(id); 
  ...
  fetchIt(){
   this.isLoading = true;
                axios.get('/karyawan/CekUserProfileAkses/RegionalAccountManagerUserAdd').then((response) => {
  ...
  if (result.value) {
                axios.delete('/karyawan/listuser/'+ item.id)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
                this.success(response.data.success);
...
this.$router.push({name:'userregionalaccountmanageredit', params: {id: this.diacak(item.id),typenya:'edit-user',rowDatanya:item }});
            }  ,
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'userregionalaccountmanagerdetail', params: {id: this.diacak(item.id),typenya:'detail-user',rowDatanya:item }});
            }  ,
            createItem() {
            this.$router.push({name:'userregionalaccountmanageradd', params: {typenya:'add-user',rowDatanya:this.dataNya}});
            } ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listuserregionalaccountmanager').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
