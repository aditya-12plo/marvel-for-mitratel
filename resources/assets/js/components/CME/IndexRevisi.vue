...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button>  
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsApprovalDocumentCMESubmitRevisi"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
var masuk = 
{
  ...
  'status' : 40,
  'statusmessage' : 'APPROVED DOKUMEN CME',
  'kata' : 'Dokumen CME Disetujui',
  'document' : 'DOKUMEN CME',
}
axios.post('/karyawan/ApprovedCMEMassal', masuk)
                    .then(response => { 
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'revisidocumentcme', params: {id: this.diacak(item.id),typenya:'revisi-submit-cme-data',rowDatanya:{datanya:this.dataNya,project:item} }});
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsApprovalDocumentCMESubmitRevisi').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
