...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button>  
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsApprovedDocumentCMEAccruedData"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
return hashids.decode(id); 
  ...
  fetchIt(){
   this.isLoading = true;
                axios.get('/karyawan/CekUserProfileAkses/HAKI').then((response) => {
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          ...
          this.$router.push({name:'accruedcmedatanya', params: {id: this.diacak(item.id),typenya:'accrued-cme-datanya',urlporelease:'/karyawan/GetCMEAccruedDataNya/'+item.project_id_accrued,rowDatanya:{datanya:this.dataNya,project:item} }});  
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsApprovedDocumentCMEAccruedData').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
