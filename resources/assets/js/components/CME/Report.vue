...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button>  
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsApprovedDocumentCMEReport"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'detailcmeapproved', params: {id: this.diacak(item.id),typenya:'approved-document-cme-data',rowDatanya:{datanya:this.dataNya,project:item} }});
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsApprovedDocumentCMEReport').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
