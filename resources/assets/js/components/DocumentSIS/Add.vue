...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen SIS Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
window.open(routeData.href, '_blank');
               ...
               dataAction () {
      if(this.typenya === "add-document-sis")
      {
           this.resetforms();
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('statusmessage', 'APPROVAL DOKUMEN SIS')
   masuk.set('document_sis', this.file_name)
   masuk.set('document', 'DOKUMEN SIS')
   masuk.set('status', 2)
                axios.post('/karyawan/AddDocumentSIS', masuk)
                    .then(response => { 
