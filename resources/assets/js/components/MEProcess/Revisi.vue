...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Perubahan Dokumen M-E Process Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
export default {
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "MEProcess-edit-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/m-e-process-revisi');
            } ,
    allcap (e, o, prop) {
        ...
        else
{
   ...
   masuk.set('m_e_process_document_lama', this.rowDatanya.project.m_e_process_document)
   masuk.set('document', 'DOKUMEN FENCE & YARD')
   masuk.set('status', 35)
                axios.post('/karyawan/RevisiDocumentMEProcess', masuk)
                    .then(response => { 
