...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen M-E Process Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen Tower Erection
</button>
	 ...
	 <div class="help-block"><ul role="alert"><li v-for="error of errorNya"><span style="color:red;">{{ error }}</span></li></ul></div>
                                 ...
                                 <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        <label for="m_e_process_date"><h4>TANGGAL DOKUMEN</h4></label>
                                        <br>
<datepicker v-model="forms.m_e_process_date" class="form-control"  :typeable="true" :format="customFormatter" placeholder="YYYY-MM-DD"></datepicker> 
  <div class="help-block"><ul role="alert"><li v-for="error of errorNya['m_e_process_date']"><span style="color:red;">{{ error }}</span></li></ul></div>
                                 ...
                                 <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                        <fieldset class="form-group">
                                            <label for="m_e_process_document"><h4>DOKUMEN</h4></label>
                                        <br>
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , meprocessid:'' ,  m_e_process_date:'' ,  m_e_process_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "MEProcess-add-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/m-e-process-add');
            } ,
    allcap (e, o, prop) {
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 32)
                axios.post('/karyawan/DocumentMEProcessPerbaikan', masuk)
                    .then(response => { 
        ...
        else
{
   ...
   masuk.set('m_e_process_document', this.file_name)
   masuk.set('document', 'DOKUMEN FENCE & YARD')
   masuk.set('status', 35)
                axios.post('/karyawan/AddDocumentMEProcess', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
