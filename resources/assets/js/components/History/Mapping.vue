...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/HistoryMappingSite"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'detailHistory', params: {id: this.diacak(item.id),typenya:'detail-history',statusnya:'MAPPING',rowDatanya:{datanya:this.dataNya,project:item} }});
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/HistoryMappingSite').then((response) => { 
                     this.isLoading = false;
                }).catch(error => {
