...
<div class="container-fluid display-page" id="display-post-category" >
 <!-- @create Modal--->
        ...
        <template slot="header" ><h4>Create New Menu</h4></template>
        <template slot="body" >
            <form method="POST" action="" @submit.prevent="submitBooks()">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control border-input" placeholder="Menu Name" v-model="forms.menu_name"  type="text" required>
                        <span class="label label-danger" v-for="error of errorNya['menu_name']">
                        {{ error }}
                    </span>
                    </div>
         ...
         <input class="form-control border-input" placeholder="Menu Code" v-model="forms.menu_code"  type="text">
                        <span class="label label-danger" v-for="error of errorNya['menu_code']">
                        {{ error }}
                    </span>
                    </div>
         ...
         <input class="form-control border-input" placeholder="Menu URL" v-model="forms.menu_url"  type="text">
                        <span class="label label-danger" v-for="error of errorNya['menu_url']">
                        {{ error }}
                    </span>
                    </div>
...
<!-- @view --->
        ...
        <template slot="body" >
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="menu_name">Menu Name :  {{forms.menu_name}}</label>
                      
                    </div>

                    <!-- form Group -->
                    <div class="form-group">
                        <label for="menu_url">Menu URL :  {{forms.menu_url}}</label>
                        
                    </div>
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="menu_code">Menu Code :  {{forms.menu_code}}</label>
    ...
    <tr>
      <td colspan="4"><button class="btn btn-success" @click.prevent="doFilter">Go</button>
          <button class="btn" @click.prevent="resetFilter">Reset</button>
                  <button type="button" class="btn btn-danger"  @click="sumSelectedItems()">
            Delete All
        </button></td>
    </tr>
</table>
...
export default {
  ...
  data () {
    ...
    formErrors:{},
    errors: new Errors() ,
    forms:new CrudForm({index:'',  id:'' , menu_code:'' , menu_name:'' , menu_url:'' , created_at:''}) ,
  ...
  methods: {
   ...
   var check = confirm("Are you sure you want to delete this row?");  
                ...
                //console.log(join_selected_values);
                    ...
                    axios.delete('/kevler/kevlerMenuDeleteAll/'+ this.join_selected_values)
                    .then(response => {
                this.errorNya = '';
  ...
  resetOptions() {
    ...
    submitBooks() {
            axios.post('/kevler/kevlerMenu', this.forms)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
   ...
   updateItem() {
            axios.put('/kevler/kevlerMenu/'+ this.forms.id , this.forms)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
            ...
            destroyItem(kode){
                axios.delete('/kevler/kevlerMenu/'+ kode)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
...
onLoading() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/kevler/kevlerMenu').then((response) => {
                    this.dataNya = response.data;
                    this.loading = false
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
