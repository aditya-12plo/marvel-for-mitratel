<template>
 <div> 
    ...
    <section class="content-header">
      ...
      <h1 align="center">
      List User AREA {{this.dataNya.area}}
      </h1>
    </section>
...
<button class="btn btn-danger" @click="sumSelectedItems()">Hapus Terpilih <i class="fa fa-trash position-right"></i></button>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/listuserregional"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
return hashids.decode(id); 
  ...
  fetchIt(){
   this.isLoading = true;
                axios.get('/karyawan/CekUserProfileAkses/RegionalUserAdd').then((response) => {
...
this.$router.push({name:'userregionaledit', params: {id: this.diacak(item.id),typenya:'edit-user',rowDatanya:item }});
            }  ,
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'userregionaldetail', params: {id: this.diacak(item.id),typenya:'detail-user',rowDatanya:item }});
            }  ,
            createItem() {
            this.$router.push({name:'userregionaladd', params: {typenya:'add-user',rowDatanya:this.dataNya}});
            } ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listuserregional').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
