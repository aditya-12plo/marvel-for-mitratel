...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Perubahan Dokumen Tower Erection Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
export default {
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "TowerErection-edit-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/tower-erection-revisi');
            } ,
    allcap (e, o, prop) {
        ...
        else
{
   ...
   masuk.set('tower_erection_document_lama', this.rowDatanya.project.tower_erection_document)
   masuk.set('document', 'DOKUMEN TOWER ERECTION')
   masuk.set('status', 33)
                axios.post('/karyawan/RevisiDocumentTowerErection', masuk)
                    .then(response => { 
