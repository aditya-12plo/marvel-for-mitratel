...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen Tower Erection Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen Curing
</button>
	 ...
	 <div class="help-block"><ul role="alert"><li v-for="error of errorNya"><span style="color:red;">{{ error }}</span></li></ul></div>
                                 ...
                                 <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        <label for="tower_erection_date"><h4>TANGGAL DOKUMEN</h4></label>
                                        <br>
<datepicker v-model="forms.tower_erection_date" class="form-control"  :typeable="true" :format="customFormatter" placeholder="YYYY-MM-DD"></datepicker> 
  <div class="help-block"><ul role="alert"><li v-for="error of errorNya['tower_erection_date']"><span style="color:red;">{{ error }}</span></li></ul></div>
                                 ...
                                 <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                        <fieldset class="form-group">
                                            <label for="tower_erection_document"><h4>DOKUMEN</h4></label>
                                        <br>
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , towererectionid:'' ,  tower_erection_date:'' ,  tower_erection_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "TowerErection-add-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/tower-erection-add');
            } ,
    allcap (e, o, prop) {
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 30)
                axios.post('/karyawan/DocumentTowerErectionPerbaikan', masuk)
                    .then(response => { 
...
if(tower_erection_date > dateNow)
{
                this.isLoading = false;
        this.modal.set('approve', false);
        ...
        else
{
   ...
   masuk.set('tower_erection_document', this.file_name)
   masuk.set('document', 'DOKUMEN M-E PROCESS')
   masuk.set('status', 33)
                axios.post('/karyawan/AddDocumentTowerErection', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
