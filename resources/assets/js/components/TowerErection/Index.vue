...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsTowerErection"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            addItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'addTowerErection', params: {id: this.diacak(item.id),typenya:'TowerErection-add-data',rowDatanya:{dataNya:this.dataNya,project:item} }});
            }  ,
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsTowerErection').then((response) => { 
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
