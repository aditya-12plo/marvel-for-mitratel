 ...
 kata,
  ...
  if (result.value) {
  	  var join_selected_values = ttl.join(","); 
                axios.delete('/karyawan/listasetDeleteAll/'+ join_selected_values)
                    .then(response => {
                this.errorNya = '';
...
return hashids.decode(id); 
  ...
  if (result.value) {
                axios.delete('/karyawan/listaset/'+ item.id)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
                this.success(response.data.success);
...
this.$router.push({name:'asetedit', params: {id: this.diacak(item.id),typenya:'detail-aset-email',rowDatanya:item }});
            }  ,
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'asetdetail', params: {id: this.diacak(item.id),typenya:'detail-aset-email',rowDatanya:item }});
            }  ,
            createItem() {
            this.$router.push({name:'asetadd', params: {typenya:'add-aset-email'}});
            } ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listaset').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
