<template>
 <div> 
    ...
    <section class="content-header">
      ...
      <h1 align="center">
      List Background Image Dr. MARVEL
      </h1>
    </section>
...
<div style="overflow-x:auto;">
  <table> 
    <tr>
      ...
      <td colspan="3"><input type="text" v-model="filterText" class="form-control" @keyup.enter="doFilter" placeholder="Nama Image"></td>
    </tr>
     <tr>
...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/listbackground"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
<!-- @create Modal -->
        ...
        <template slot="header" ><h4 align="center">Upload Image</h4></template>
        <template slot="body" >
...
<div class="col-xl-4 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        ...
                                        <br>
<input v-model="forms.sequence" type="number" class="form-control" placeholder="Sequence" min="1" required>
...
<!-- @update Modal -->
        ...
        <template slot="header" ><h4 align="center">Edit Image</h4></template>
        <template slot="body" >
...
Vue.component('image-uploadnya', require('./Banner.vue'))
...
export default {
  ...
  data () {
   ...
   forms:new CrudForm({index:'',  id:'', sequence:'' , name:'' , image:''  , created_at:''}) ,
     dataNya: {name : '', level:''},
    perPage: 10,
    loading: false,
      fields: [
       {
        ...
        {
          name: '__component:image-uploadnya',
		  title: 'Images',
		  titleClass: 'text-center',
          dataClass: 'text-center'
        },
  ...
  let masuk = new FormData();
   ...
   masuk.set('image', this.filename)
              axios.post('/karyawan/addbackground' , masuk)
                    .then(response => {
...
return hashids.decode(id); 
  ...
  if (result.value) {
                axios.delete('/karyawan/deletebackground/'+ item.id+'/'+item.image)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
                this.success(response.data.success);
           ...
           downloadDataFile(){ 
axios({
  url: '/public/img/'+this.forms.image,
  method: 'GET', 
  ...
  link.href = url;
  link.setAttribute('download', this.forms.image);
...
axios({
  ...
  const link = document.createElement('a');
  link.href = url;
  link.setAttribute('download', item.image);
  document.body.appendChild(link);
  link.click();
}); 
            }  , 
  ...
  let masuk = new FormData();
   ...
   masuk.set('sequence', this.forms.sequence)
   masuk.set('name', this.forms.name)
   masuk.set('image_old', this.forms.image)
   masuk.set('image', this.filename)
              axios.post('/karyawan/updatebackground' , masuk)
                    .then(response => {
if(response.data.success)
{       
  this.isLoading = false;
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listbackground').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
