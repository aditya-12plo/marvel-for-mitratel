<template>
 <div> 
    ...
    <section class="content-header">
      ...
      <h1 align="center">
      List Image Slide Show Dr. MARVEL
      </h1>
    </section>
...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/listslideshow"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
export default {
  ...
  let masuk = new FormData();
   ...
   masuk.set('image', this.filename)
              axios.post('/karyawan/addslideshow' , masuk)
                    .then(response => {
...
return hashids.decode(id); 
  ...
  if (result.value) {
                axios.delete('/karyawan/deleteslideshow/'+ item.id+'/'+item.image)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
                this.success(response.data.success);
...
axios({
  ...
  let masuk = new FormData();
   ...
   masuk.set('image', this.filename)
              axios.post('/karyawan/updateslideshow' , masuk)
                    .then(response => {
if(response.data.success)
{       
  this.isLoading = false;
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listslideshow').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
