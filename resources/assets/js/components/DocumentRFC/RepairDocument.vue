...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <div class="content-header" align="center">Form Perubahan Dokumen RFC Project ID {{this.rowDatanya.project.projectid}}</div>
        </div>
    </div>
...
this.file_name = this.$refs.file_name.files[0]; 
 let masuk = new FormData();
   masuk.set('id', this.rowDatanya.project.documentrfcid)
   masuk.set('project_id', this.rowDatanya.project.id)
   masuk.set('projectid', this.rowDatanya.project.projectid)
   masuk.set('namafile', this.rowDatanya.project.document_rfc)
   masuk.set('document_rfc', this.file_name) 
axios.post('/karyawan/uploaddokumenRFC' , masuk)
                    .then(response => {
if(response.data.success)
{
this.isLoading = false;
    this.rowDatanya.project.document_rfc = response.data.namafilenya;
    this.file_name = '';
    this.errorNya = '';
...
'Ooppss?',
 kata,
  'question'
            ...
            backLink() {
			  this.$router.push('/repair-documents-rfc');
            } ,
    allcap (e, o, prop) {
...
*/
   ...
   masuk.set('status', 11)
                axios.post('/karyawan/RevisiDocumentRFC', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
...
DeleteRFC(id,projectid,documentrfc){
    var masuk = {kode:id , projectid:projectid , document_rfc:documentrfc};
                axios.post('/karyawan/DeleteRFC',masuk).then((response) => {
                    return response.data.success;
                });
            }, 
  GetKomunikasi(id){
