...
<button class="btn btn-danger" @click="sumSelectedItems()">Approved Terpilih <i class="ft-check-circle position-right"></i></button>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsApprovalDocumentRFC"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'approvaldocumentrfc', params: {id: this.diacak(item.id),typenya:'approval-document-rfc',rowDatanya:{datanya:this.dataNya,project:item} }});
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsApprovalDocumentRFC').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
