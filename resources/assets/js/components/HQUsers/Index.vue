...
return hashids.decode(id); 
  ...
  fetchIt(){
   this.isLoading = true;
                axios.get('/karyawan/CekUserProfileAkses/HQUserAdd').then((response) => {
...
this.$router.push({name:'userhqedit', params: {id: this.diacak(item.id),typenya:'edit-user-hq',rowDatanya:{user:item,admin:this.dataNya} }});
            }  ,
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'userhqdetail', params: {id: this.diacak(item.id),typenya:'detail-user-hq',rowDatanya:item }});
            }  ,
            createItem() {
            this.$router.push({name:'userhqadd', params: {typenya:'add-user-hq',rowDatanya:this.dataNya}});
            } ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listuserhq').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
