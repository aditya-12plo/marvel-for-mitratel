...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen Excavation Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen Site Opening
</button>
	 ...
	 <div class="help-block"><ul role="alert"><li v-for="error of errorNya"><span style="color:red;">{{ error }}</span></li></ul></div>
                                ...
                                <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        <label for="excavation_date"><h4>TANGGAL DOKUMEN</h4></label>
                                        <br>
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , excavationid:'' ,  excavation_date:'' ,  excavation_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "excavation-add-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/excavation-add');
            } ,
    allcap (e, o, prop) {
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 22)
                axios.post('/karyawan/DocumentSiteOpeningPerbaikan', masuk)
                    .then(response => { 
