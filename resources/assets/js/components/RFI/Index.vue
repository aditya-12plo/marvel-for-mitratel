...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsRfiDetail"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
Vue.use(VueEvents)
Vue.component('add-detail-custom-actions', require('../Button/AddDetailActions.vue'))
  ...
  data () {
    ...
    loading: false,
      ...
      titleClass: 'text-center',
        ...
        {
          name: '__component:add-detail-custom-actions',
          title: 'Actions',
          titleClass: 'text-center',
          dataClass: 'text-center'
        }
...
return hashids.decode(id); 
  ...
  fetchIt(){
   this.isLoading = true;
                axios.get('/karyawan/CekUserProfileAkses/DocumentRfiDetail').then((response) => {
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            addItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'addRfiDetail', params: {id: this.diacak(item.id),typenya:'RfiDetail-add-data',rowDatanya:{dataNya:this.dataNya,project:item} }});
            }  ,
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsRfiDetail').then((response) => { 
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
 ...
 created: function() {
  let self = this;
            ...
            this.$root.$on('additem', function(data,index){ 
