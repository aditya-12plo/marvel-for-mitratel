...
<button class="btn btn-danger" @click="sumSelectedItems()">Approved Terpilih <i class="ft-check-circle position-right"></i></button>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsSubmitCMEApproval"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
this.$router.push({name:'approvaldocumentrfidetail', params: {id: this.diacak(item.id),typenya:'approval-document-rfi-detail',rowDatanya:{datanya:this.dataNya,project:item} }});             
            }  ,
   ...
   var masuk = 
{
  ...
  'document' : 'DOKUMEN CME',
                ...
                axios.post('/karyawan/SubmitRFIDetailMassal', masuk)
                    .then(response => { 
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsSubmitCMEApproval').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
