...
<button class="btn btn-danger" @click="sumSelectedItems()">Approved Terpilih <i class="ft-check-circle position-right"></i></button>
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsApprovalDocumentSITAC"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
var masuk = 
{
  ...
  'status' : 10,
  'statusmessage' : 'APPROVED DOKUMEN SITAC',
  'kata' : 'Dokumen SITAC Disetujui',
  'document' : 'DOKUMEN SITAC',
}
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'approvaldocumentsitac', params: {id: this.diacak(item.id),typenya:'approval-document-sitac',rowDatanya:{datanya:this.dataNya,project:item} }});
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsApprovalDocumentSITAC').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
