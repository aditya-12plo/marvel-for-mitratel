...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsBOQApproval"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
return hashids.decode(id); 
  ...
  fetchIt(){
   this.isLoading = true;
                axios.get('/karyawan/CekUserProfileAkses/DocumentBOQApprovalManager').then((response) => {
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          ...
          this.$router.push({name:'approvalboq', params: {id: this.diacak(item.id),typenya:'approval-boq',rowDatanya:{datanya:this.dataNya,project:item} }}); 
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsBOQApproval').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
