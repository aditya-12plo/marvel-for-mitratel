...
<div style="overflow-x:auto;">
  ...
  <tr>
    ...
    <tr>
      ...
      <td colspan="3"><input type="text" v-model="filterText" class="form-control" @keyup.enter="doFilter" placeholder="Kode BOQ"></td>
    </tr>
     <tr>
      <td colspan="4" style="padding-top: 1%;"></td>
    </tr>
...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsSubmitBOQVerifikasi"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
return hashids.decode(id); 
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          ...
          this.$router.push({name:'approvedboqverifikasisubmit', params: {id: this.diacak(item.id),typenya:'approved-boq-verifikasi-submit',urlverifikasi:'/karyawan/GetBOQVerifikasi/'+item.project_id_boq,rowDatanya:{datanya:this.dataNya,project:item} }});  
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsSubmitBOQVerifikasi').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
