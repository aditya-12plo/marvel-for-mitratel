...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsSubmitBOQApproved"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
return hashids.decode(id); 
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          ...
          this.$router.push({name:'approvedboq', params: {id: this.diacak(item.id),typenya:'approved-boq',rowDatanya:{datanya:this.dataNya,project:item} }}); 
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsSubmitBOQApproved').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
