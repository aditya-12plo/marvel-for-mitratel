  ...
  <loading :show="isLoading"></loading>
   <vue-toast ref='toast'></vue-toast>
    <section class="content-header">
      ...
      <h1 align="center">
      Repair BOQ Area {{this.dataNya.area}}
      </h1>
    </section>
...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsSubmitBOQRepair"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
return hashids.decode(id); 
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          ...
          this.$router.push({name:'approvalboqrepair', params: {id: this.diacak(item.id),typenya:'approval-boq-repair',rowDatanya:{datanya:this.dataNya,project:item} }}); 
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsSubmitBOQRepair').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
