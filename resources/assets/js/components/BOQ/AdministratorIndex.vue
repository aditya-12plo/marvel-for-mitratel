...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsSubmitAllBOQ"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
return hashids.decode(id); 
  ...
  resetOptions() {
        ...
        showTime(kode,pesan) {
          ...
          this.$router.push({name:'approvaleditadmindata', params: {id: this.diacak(item.id),typenya:'edit-boq-data',rowDatanya:{datanya:this.dataNya,project:item} }}); 
            }  ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsSubmitAllBOQ').then((response) => {
                     this.isLoading = false;
                }).catch(error => {
        if (! _.isEmpty(error.response)) {
