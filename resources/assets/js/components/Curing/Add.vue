...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen Curing Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen Pouring
</button>
	 ...
	 <div class="help-block"><ul role="alert"><li v-for="error of errorNya"><span style="color:red;">{{ error }}</span></li></ul></div>
                                 ...
                                 <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        <label for="curing_date"><h4>TANGGAL DOKUMEN</h4></label>
                                        <br>
<datepicker v-model="forms.curing_date" class="form-control"  :typeable="true" :format="customFormatter" placeholder="YYYY-MM-DD"></datepicker> 
  <div class="help-block"><ul role="alert"><li v-for="error of errorNya['curing_date']"><span style="color:red;">{{ error }}</span></li></ul></div>
                                     ...
                                     <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                        <fieldset class="form-group">
                                            <label for="curing_document"><h4>DOKUMEN</h4></label>
                                        <br>
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , rebaringid:'' ,  curing_date:'' ,  excavation_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "curing-add-data")
      {
           this.resetforms();
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 28)
                axios.post('/karyawan/DocumentCuringPerbaikan', masuk)
                    .then(response => { 
        ...
        else
{
   ...
   masuk.set('curing_document', this.file_name)
   masuk.set('document', 'DOKUMEN TOWER ERECTION')
   masuk.set('status', 31)
                axios.post('/karyawan/AddDocumentCuring', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
