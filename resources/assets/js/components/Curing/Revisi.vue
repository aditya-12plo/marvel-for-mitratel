...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , pouringid:'' ,  curing_date:'' ,  curing_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "curing-edit-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/curing-revisi');
            } ,
    allcap (e, o, prop) {
        ...
        else
{
   ...
   masuk.set('curing_document_lama', this.rowDatanya.project.curing_document)
   masuk.set('document', 'DOKUMEN CURING')
   masuk.set('status', 31)
                axios.post('/karyawan/RevisiDocumentCuring', masuk)
                    .then(response => { 
