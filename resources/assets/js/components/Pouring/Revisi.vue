...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Perubahan Dokumen Pouring Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , pouringid:'' ,  pouring_date:'' ,  pouring_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "pouring-edit-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/pouring-revisi');
            } ,
    allcap (e, o, prop) {
        ...
        else
{
   ...
   masuk.set('pouring_document_lama', this.rowDatanya.project.pouring_document)
   masuk.set('document', 'DOKUMEN POURING')
   masuk.set('status', 29)
                axios.post('/karyawan/RevisiDocumentPouring', masuk)
                    .then(response => { 
