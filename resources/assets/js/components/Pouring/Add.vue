...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen Pouring Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen Rebaring
</button>
	 ...
	 <div class="help-block"><ul role="alert"><li v-for="error of errorNya"><span style="color:red;">{{ error }}</span></li></ul></div>
                                ...
                                <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        <label for="pouring_date"><h4>TANGGAL DOKUMEN</h4></label>
                                        <br>
<datepicker v-model="forms.pouring_date" class="form-control"  :typeable="true" :format="customFormatter" placeholder="YYYY-MM-DD"></datepicker> 
  <div class="help-block"><ul role="alert"><li v-for="error of errorNya['pouring_date']"><span style="color:red;">{{ error }}</span></li></ul></div>
                                    ...
                                    <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                        <fieldset class="form-group">
                                            <label for="pouring_document"><h4>DOKUMEN</h4></label>
                                        <br>
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , rebaringid:'' ,  pouring_date:'' ,  excavation_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "pouring-add-data")
      {
           this.resetforms();
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 26)
                axios.post('/karyawan/DocumentPouringPerbaikan', masuk)
                    .then(response => { 
        ...
        else
{
   ...
   masuk.set('pouring_document', this.file_name)
   masuk.set('document', 'DOKUMEN CURING')
   masuk.set('status', 29)
                axios.post('/karyawan/AddDocumentPouring', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
