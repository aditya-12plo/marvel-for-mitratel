...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Perubahan Dokumen Fence & Yard Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
   ...
   <div class="help-block"><ul role="alert"><li v-for="error of errorNya"><span style="color:red;">{{ error }}</span></li></ul></div>
                                 ...
                                 <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        <label for="fence_yard_date"><h4>TANGGAL DOKUMEN</h4></label>
                                        <br>
<datepicker v-model="forms.fence_yard_date" class="form-control"  :typeable="true" :format="customFormatter" placeholder="YYYY-MM-DD"></datepicker> 
  <div class="help-block"><ul role="alert"><li v-for="error of errorNya['fence_yard_date']"><span style="color:red;">{{ error }}</span></li></ul></div>
                                 ...
                                 <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                        <fieldset class="form-group">
                                            <label for="fence_yard_document"><h4>DOKUMEN</h4></label>
                                        <br>
<label id="projectinput8" class="file center-block">
            ...
            <span class="file-custom"></span>
                ...
                <br>
 <a v-bind:href="'/files/'+this.rowDatanya.project.projectid+'/'+this.rowDatanya.project.fence_yard_document" target="_blank"><button type="button" class="btn btn-success"><i class="ft-download"></i> Download</button></a>  
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , fenceyardid:'' ,  fence_yard_date:'' ,  fence_yard_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "FenceYard-edit-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/fence-yard-revisi');
            } ,
    allcap (e, o, prop) {
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
var fence_yard_date = this.customFormatter(this.forms.fence_yard_date) 
var dateNow = new Date().toISOString().slice(0,10)

if(fence_yard_date > dateNow)
{
        ...
        else
{
   ...
   masuk.set('id', this.rowDatanya.project.fenceyardid) 
   masuk.set('project_id', this.rowDatanya.project.id) 
   masuk.set('projectid', this.rowDatanya.project.projectid)
   masuk.set('fence_yard_date', fence_yard_date)
   masuk.set('fence_yard_document', this.file_name)
   masuk.set('fence_yard_document_lama', this.rowDatanya.project.fence_yard_document)
   masuk.set('document', 'DOKUMEN RFI & BAUT')
   masuk.set('status', 37)
                axios.post('/karyawan/RevisiDocumentFenceYard', masuk)
                    .then(response => { 
