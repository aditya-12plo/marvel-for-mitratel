...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsFenceYardRevisi"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
...
this.$router.push({name:'revisiFenceYard', params: {id: this.diacak(item.id),typenya:'FenceYard-edit-data',rowDatanya:{dataNya:this.dataNya,project:item} }});
            }  ,
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsFenceYardRevisi').then((response) => { 
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
