...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen Fence & Yard Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen M-E Process
</button>
...
export default {
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "FenceYard-add-data")
      {
           this.resetforms();
...
return hashids.decode(id); 
  ...
  timer: 1500
            ...
            backLink() {
			   this.$router.push('/fence-yard-add');
            } ,
    allcap (e, o, prop) {
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 34)
                axios.post('/karyawan/DocumentFenceYardPerbaikan', masuk)
                    .then(response => { 
        ...
        else
{
   ...
   masuk.set('status', 37)
                axios.post('/karyawan/AddDocumentFenceYard', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
