            ...
            <form method="POST" action="" @submit.prevent="submitBooks()">
                <div class="modal-body">
                   <!-- form Group -->
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="level">Posisi Karyawan</label>
						...
						<option v-for="pilih in pilihan" :value="pilih">{{ pilih }}</option>
                         ...
                         <span class="label label-danger" v-for="error of errorNya['level']">
                        {{ error }}
                    </span>
                    </div>
										
					 <!-- form Group -->
                    <div class="form-group" v-if="forms.level=='MANAGER AREA' || forms.level=='GENERAL MANAGER AREA' || forms.level=='MANAGER UBIS' || forms.level=='STAFF PROCUREMENT'">
                        <label for="regional">Area</label>
                        <input class="form-control border-input" placeholder="Area" v-model="forms.area"  type="text" required>
                        <span class="label label-danger" v-for="error of errorNya['area']">
                        {{ error }}
                    </span>
                    </div>
										
					 <!-- form Group -->
                    <div class="form-group" v-if="forms.level=='MANAGER UBIS'">
                        <label for="regional">Area 2</label>
                        <input class="form-control border-input" placeholder="Area 2" v-model="forms.area2"  type="text" required>
                        <span class="label label-danger" v-for="error of errorNya['area2']">
                        {{ error }}
                    </span>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control border-input" placeholder="Password" v-model="forms.password"  type="password" required>
...
<!-- @view --->
        ...
        <template slot="body" >
                <div class="modal-body">
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="suspend">Posisi :  {{forms.level}}</label>
                        
                    </div>

                    <!-- form Group -->
                  <div class="form-group" v-if="forms.level=='MANAGER AREA' || forms.level=='GENERAL MANAGER AREA' || forms.level=='MANAGER UBIS' || forms.level=='STAFF PROCUREMENT'">
                        <label for="suspend">Area :  {{forms.area}}</label>
                        
                    </div>

                    <!-- form Group -->
                  <div class="form-group" v-if="forms.level=='MANAGER UBIS'">
                        <label for="area2">Area 2 :  {{forms.area2}}</label>
...
<!-- @update --->
        ...
        <template slot="header" ><h4>Edit Data User</h4></template>
        <template slot="body" >
            ...
            <form method="POST" action="" @submit.prevent="updateItem()">
                <div class="modal-body">
				   ...
				   <!-- form Group -->
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="level">Level Karyawan - {{forms.level}}</label>
                        <select class="form-control border-input" v-model="forms.level">
						<option value="">Choose One</option>
						<option v-for="pilih in pilihan" :value="pilih">{{ pilih }}</option>
						</select>
                         <span class="label label-danger" v-for="error of errorNya['level']">
                    ...
                    <div class="form-group">
                        <label for="password">Password</label>
                        <input class="form-control border-input" placeholder="Password" v-model="forms.password"  type="password">
                        <span class="label label-danger" v-for="error of errorNya['password']">
                        {{ error }}
                    </span>
                    </div>
...
export default {
  ...
  data () {
	...
	GetLevel:'',  
	file_name:'',
	pilihan: ['OFFICER PSC','MANAGER AREA','GENERAL MANAGER AREA','MANAGER UBIS','GENERAL MANAGER UBIS','EXECUTIVE GENERAL MANAGER UBIS','STAFF PROCUREMENT','OFFICER PROCUREMENT','MANAGER PROCUREMENT','DIRECTOR'],
    errors: new Errors() ,
   forms:new CrudForm({index:'',  id:'' , name:'' , email:'' , level:'',  area:'',  area2:'',  password:'' , created_at:''}) ,
  ...
  symbol: " M",precision: 0,format: "%v %s"})
    ...
    submitBooks() {
	 axios.post('/karyawan/Addkaryawan', this.forms)
                    .then((response) => {
                this.errorNya = '';
                this.resetFilter();
...
axios.put('/karyawan/Updatekaryawan/'+ this.forms.id , this.forms)
            ...
            destroyItem(kode){
                axios.delete('/karyawan/Deletekaryawan/'+ kode)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listuser').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
