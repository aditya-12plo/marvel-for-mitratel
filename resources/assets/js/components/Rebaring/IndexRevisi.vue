...
<button class="btn btn-warning" @click.prevent="resetFilter">Reset Form <i class="fa fa-refresh position-right"></i></button> 
    ...
    <vuetable ref="vuetable"
      api-url="/karyawan/GetJobsRebaringRevisi"
      :fields="fields"
      pagination-path=""
      :per-page="perPage"
      :css="css.table"
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            editItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'revisirebaring', params: {id: this.diacak(item.id),typenya:'rebaring-edit-data',rowDatanya:{dataNya:this.dataNya,project:item} }});
            }  ,
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/GetJobsRebaringRevisi').then((response) => { 
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
