...
<section class="basic-elements">
    <div class="row">
        <div class="col-sm-12">
            <h1 align="center">Form Penambahan Dokumen Rebaring Project ID {{this.rowDatanya.project.projectid}}</h1>
        </div>
    </div>
...
<button type="button" @click="mundur()" class="btn btn-raised btn-success">
    <i class="ft-corner-left-up"></i> Perbaiki ke Dokumen Excavation
</button>
	 ...
	 <div class="help-block"><ul role="alert"><li v-for="error of errorNya"><span style="color:red;">{{ error }}</span></li></ul></div>
                                ...
                                <div class="col-xl-6 col-lg-6 col-md-12 mb-1">
                                    <fieldset class="form-group">
                                        <label for="rebaring_date"><h4>TANGGAL DOKUMEN</h4></label>
                                        <br>
<datepicker v-model="forms.rebaring_date" class="form-control"  :typeable="true" :format="customFormatter" placeholder="YYYY-MM-DD"></datepicker> 
...
export default {
 ...
 data () {
  ...
  modal:new CrudModal({komunikasiproject: false,approve: false,drop: false}),
    formErrors:{},
     ...
     message:'',
	GetLevel:'', 
	forms: new CrudForm({id:'' , project_id:'' , rebaringid:'' ,  rebaring_date:'' ,  excavation_document:'' , created_at:''}), 
    errors: new Errors() ,
    errorNya: [],
    komunikasi:[] ,
    dataNya: {name : '', level:''}, 
 ...
 watch: {
      ...
      return moment(date).format('YYYY-MM-DD');
               ...
               dataAction () {
      if(this.typenya === "rebaring-add-data")
      {
           this.resetforms();
...
'Ooppss?',
 kata,
  ...
  if (result.value) {
   ...
   masuk.set('status', 24)
                axios.post('/karyawan/DocumentRebaringPerbaikan', masuk)
                    .then(response => { 
  ...
  if (result.value) {
var rebaring_date = this.customFormatter(this.forms.rebaring_date) 
var dateNow = new Date().toISOString().slice(0,10)

if(rebaring_date > dateNow)
{
                this.isLoading = false;
        this.modal.set('approve', false);
        ...
        else
{
   ...
   masuk.set('rebaring_date', rebaring_date)
   masuk.set('rebaring_document', this.file_name)
   masuk.set('document', 'DOKUMEN POURING')
   masuk.set('status', 27)
                axios.post('/karyawan/AddDocumentRebaring', masuk)
                    .then(response => { 
                      if(response.data.success)
                      {
                 this.success(response.data.success);
                 ...
                 this.backLink();
                      ...
                      else
                      {
                         this.modal.set('approve', false);
                          this.isLoading = false; 
                        this.errorNya = {rebaring_document:[response.data.error]};
                      }
                    })
                    .catch(error => {
                    if (! _.isEmpty(error.response)) {
