...
this.$router.push({name:'userhakiedit', params: {id: this.diacak(item.id),typenya:'edit-user-haki',rowDatanya:{user:item,admin:this.dataNya} }});
            }  ,
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'userhakidetail', params: {id: this.diacak(item.id),typenya:'detail-user-haki',rowDatanya:item }});
            }  ,
            createItem() {
            this.$router.push({name:'userhakiadd', params: {typenya:'add-user-haki',rowDatanya:this.dataNya}});
            } ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listuserhaki').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
