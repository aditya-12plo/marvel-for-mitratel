...
<div style="overflow-x:auto;">
  ...
  <tr>
    ...
    <tr>
      ...
      <td colspan="3"><input type="text" v-model="filterText" class="form-control" @keyup.enter="doFilter" placeholder="Email / Nama "></td>
    </tr>
     <tr>
      <td colspan="4" style="padding-top: 1%;"></td>
    </tr>
 ...
 kata,
  ...
  if (result.value) {
  	  var join_selected_values = ttl.join(","); 
                axios.delete('/karyawan/listbusdevDeleteAll/'+ join_selected_values)
                    .then(response => {
                this.errorNya = '';
...
return hashids.decode(id); 
  ...
  if (result.value) {
                axios.delete('/karyawan/listbusdev/'+ item.id)
                    .then(response => {
                this.errorNya = '';
                this.resetFilter();
                this.success(response.data.success);
...
this.$router.push({name:'busdevedit', params: {id: this.diacak(item.id),typenya:'detail-busdev-email',rowDatanya:item }});
            }  ,
            viewItem(item ,index = this.indexOf(item)){
                this.$router.push({name:'busdevdetail', params: {id: this.diacak(item.id),typenya:'detail-busdev-email',rowDatanya:item }});
            }  ,
            createItem() {
            this.$router.push({name:'busdevadd', params: {typenya:'add-busdev-email'}});
            } ,
        doFilter () {
...
onLoading() {
    ...
    onLoadingError() {
               this.isLoading = true;
                axios.get('karyawan/listbusdev').then((response) => {
                    this.dataNya = response.data;
                     this.isLoading = false;
                }).catch(error => {
				if (! _.isEmpty(error.response)) {
